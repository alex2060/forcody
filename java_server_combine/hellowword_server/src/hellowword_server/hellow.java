...
public class hellow {
	...
	public static void main(String[] args) {
		...
		try {
			server = HttpServer.create(new InetSocketAddress(4253), 0);
			System.out.println(":running on 4251");
			
		} catch (IOException e) {
			// TODO Auto-generated catch block
			System.out.println("lsof -n -i TCP:4521 and kill whats ever on that or change to running port");
			e.printStackTrace();
		}
		...
		fornt.setHandler((he) -> {
			...
			String fpayload = "\n"
					...
					+ "        <div id = \"place_to_stick_stuff\"><label for=\"uname\" class=\"text-white\">NO trade made </label></div>\n"
					+ "        </br>\n"
					...
					+ "  if (output[1]==\"success\") {\n"
					+ "\n"
					+ "  document.getElementById(\"place_to_stick_stuff\").innerHTML = \"<label for=\\\"uname\\\" class=\\\"text-white\\\">\"+\"amount of money left = \"+output[2]+\"</br>trade id = \"+output[3]+\"</label>\";\n"
					+ "  }\n"
					+ "  else{\n"
